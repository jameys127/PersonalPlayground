...
const createProject = async (title, github, link, short, img, description) => {
    const client = await pool.connect();
    try{
        await client.query('BEGIN');
        const slug = createSlug(title);
        const query = `
            INSERT INTO projects (title, github, link, short, description, slug)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING id;
        `;
        // insert img into seperate table.
        const values = [title, github, link, short, description, slug]
...
const getAllProjects = async () => {
    const query = `
        SELECT p.id, p.title, p.github, p.link, p.short, p.description, p.slug,
        COALESCE(
            JSON_AGG(i.img_url) FILTER (WHERE i.img_url IS NOT NULL), '[]'
        ) AS images
        FROM projects p
        LEFT JOIN project_images i ON p.id = i.project_id
        GROUP BY p.id, p.title, p.github, p.link, p.short, p.description, p.slug
        ORDER BY p.id;
    `;
    try{
...
const updateProject = async (id, title, github, link, short, img, description) => {
    const client = await pool.connect();
    try{
        await client.query('BEGIN')
        const replaceImgs = `
        ...
        const slug = createSlug(title);
        const query = `
            UPDATE projects SET title = $1, github = $2, link = $3, short = $4, description = $5, slug = $6
            WHERE id = $7;
        `;
        const values = [title, github, link, short, description, slug, id];
        const result = await client.query(query, values);
        await client.query('COMMIT');
        return result.rowCount;
    }catch (err){
